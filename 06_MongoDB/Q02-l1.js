// 데이터베이스를 tutorial로 설정하세요
use tutorial;

// users 컬렉션에 username이 smith인 문서를 저장하세요.
// db는 현재 사용중인 데이터베이스를 설정한 전역변수
// insert() : 문서(document) 삽입
...
// MongoInvalidArgumentError: 업데이트 연산자가 필요
// db.users.updateOne({username:"smith"}, {country:"Canada"});
// db.users.find({country:"Canada"});

// users 컬렉션에서 username이 smith인 문서를 {username: "smith", country: Canada}로 대체하고 확인하세요
// MongoInvalidArgumentError: 업데이트 연산자가 필요
// db.users.update({username:"smith"}, {username: "smith", country:"Canada"});
...
// user 컬렉션에서 username이 smith인 문서를 삭제하고, 삭제여부를 확인하세요
...
// users 컬렉션을 삭제하세요
// drop(): 컬렉션 삭제
// tutorial 안에 users 컬렉션 밖에 없었기 때문에 tutorial도 같이 사라지게 됨
db.users.drop();
