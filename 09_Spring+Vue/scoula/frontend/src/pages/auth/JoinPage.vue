...
import authApi from '@/api/authApi.js'; // 인증 관련 API 함수들
...
const checkUsername = async () => {
  ...
  // 제출 버튼이 비활성화된 상태면 ID가 사용중이란 에러 메세지 출력
...
const join = async () => {
  ...
  if (avatar.value.files.length > 0) {
    // 첨부파일이 업로드 된 경우 member 객체에 추가
    member.avatar = avatar.value.files[0];
  }

  try {
    await authApi.create(member); // 회원가입 API 호출
    router.push({ name: 'home' }); // 회원가입 성공 시, 홈 화면으로 이동
  } catch (e) {
    console.error(e); // 에러 발생시 콘솔에 출력
  }
};
</script>
...
<template>
  <div class="mt-5 mx-auto" style="width: 500px">
    ...
    <form @submit.prevent="join">
      <!-- prevent -> default 액션(서버 전송)을 막고 join 메소드만 부른다 -->
      <div class="mb-3 mt-3">
