...
const removeFile = async (no, filename) => {
  ...
  const ix = attachments.value.findIndex((f) => f.no === no); // 첨부파일의 no 값을 비교해서 우리가 찾는 첨부파일의 인덱스를 가져온다.
  attachments.value.splice(ix, 1); // splice(시작 인덱스, 몇 개를 삭제할지 개수, (삽입할 아이템))
};
...
const submit = async () => {
  ...
  if (files.value.files.length > 0) {
    ...
    // reactive 값은 바로 접근. ref 값은 .value로 접근
  }
...
// 게시글 데이터 초기화
const reset = () => {
  // orgArticle 값에 기존 게시글 데이터를 저장해둔 후 reset 시 다시 불러온다
